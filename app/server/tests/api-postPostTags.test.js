...
describe('Testing user\'s ability to POST Post Tags', () => {
    ...
    beforeAll(async () => {
        ...
        await community.setTagsForCommunity(communityName, tag2, loginResponse.body.token);
    ...
    test('Trying to tag the post again with the same tag returns 403', async () => {
        response = await post.createPostTags(postResponse.body._id, tag1, loginResponse.body.token);
    ...
    test('After trying to add a second tag, user cannot retrieve that tag as well (AKA tag not added)', async () => {
        await post.createPostTags(postResponse.body._id, tag1, loginResponse.body.token); // Just in case first test doesn't run
        response = await post.createPostTags(postResponse.body._id, tag2, loginResponse.body.token);
        expect(response.status).toBe(403);
        response = await post.getPostTags(postResponse.body._id, loginResponse.body.token);
        expect(response.status).toBe(200);
    ...
    test('User cannot add a tag to a post if it is not defined in the community', async () => {
        response = await post.createPostTags(postResponse.body._id, 'Undefined Tag', loginResponse.body.token);
        expect(response.status).toBe(403);
        response = await post.getPostTags(postResponse.body._id, loginResponse.body.token);
        expect(response.status).toBe(200);
        expect(response.body).not.toEqual(
    ...
    test('User cannot tag post when providing a non-existant post id - returns 404', async () => {
        response = await post.createPostTags('kdls;hiowkl;sdkhflw', 'bad post id', loginResponse.body.token);
        expect(response.status).toBe(404);
    });

    test('User cannot tag post when providing an invalid user token - returns 403', async () => {
        response = await post.createPostTags(postResponse.body._id, 'bad token', 'kdslfhsdfhiuwhl');
        expect(response.status).toBe(403);
    });
});
