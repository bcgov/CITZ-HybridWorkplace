...
/**
 * Given a list of disallowedFields, check that the request body does
 * NOT include them. If it does, throw an error.
 */
const checkPatchQuery = (requestBody, document, disallowedFields) => {
  // eslint-disable-next-line prefer-const
  let query = { $set: {} };

  Object.keys(requestBody).forEach((key) => {
    if (disallowedFields.includes(key) || key === "_id")
